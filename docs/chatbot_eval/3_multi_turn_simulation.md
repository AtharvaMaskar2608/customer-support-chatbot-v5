# Multi-Turn Simulation (/guides/guides-multi-turn-simulation)
...
### What Simulation Gives You [#what-simulation-gives-you]
...
* **Forward-looking** — Every simulation runs against your *current* application, so you catch regressions in real time
...
A [`ConversationalGolden`](/docs/conversation-simulator#simulate-a-conversation) defines *what* a conversation should be about, without prescribing the exact messages. It has three key fields:
...
## The Model Callback [#the-model-callback]
...
### Minimal Callback [#minimal-callback]
...
### Callback with Conversation History [#callback-with-conversation-history]
...
### Callback with Thread ID [#callback-with-thread-id]
...
### Controlling Conversation Length [#controlling-conversation-length]
...
### Custom Simulator Model [#custom-simulator-model]
...
### Starting from Existing Turns [#starting-from-existing-turns]
...
### Designing Effective Scenarios [#designing-effective-scenarios]
...
## From Simulation to Evaluation [#from-simulation-to-evaluation]
