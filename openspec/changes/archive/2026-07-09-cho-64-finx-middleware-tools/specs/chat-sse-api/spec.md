# chat-sse-api — delta for finx-middleware-tools
...
**Reason**: Report results no longer flow back through the model — they render directly as frontend widgets, so there is no paused turn to resume.
**Migration**: Use the new report execution endpoint below; the frontend appends a plain-text marker turn to its durable history for conversational continuity.
